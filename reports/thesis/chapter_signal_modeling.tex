...
$t$ and $t+1$ with $M$ maxima in frame $t$ and $N$ maxima in frame $t+1$. In
\cite{mcaulay1986speech}  the parameters are the instantaneous amplitude, phase
and frequency and are indexed by frequency as $\omega_0^{t}, \dotsc,
\omega_{M-1}^{t}$ and $\omega_0^{t+1}, \dotsc, \omega_{N-1}^{t+1}$, but here we
allow for an arbitrary set of parameters $\theta_0^{t}, \dotsc,
\theta_{M-1}^{t}$ and $\theta_0^{t+1}, \dotsc,
\theta_{N-1}^{t+1}$, such as the coefficients of a phase polynomial. Define a
...
\[
    C = \theta^{t} \otimes_{\mathcal{D}} \theta^{t+1}
\]
so that the $i$th row and $j$th column contain $C_{i,j} = \mathcal{D} \left(
\theta_{i}^{t},\theta_{j}^{t+1} \right)$.  For each $l \in \left[0 \dotsc L-1
...
their frame number $t$, we make $t$ part of the parameter set so that it can be
...
distances $C \in \mathbb{R}^{3 \times 4 \times 2}$ whose entry $C_{i,j,t}$
represents the cost of travelling on the path connecting the $i$th node in layer
0, the $j$th node in layer 1 and the $t$th node in layer 2. This is the greedy
...
noticing that if this is true, there will be $L$ edges between frames $t$ and
$t+1$. We constrain the number of paths going from edges
$\Gamma_{t}$ in frame $t$ to $\Gamma_{t+1}$ by forming the vector
\[
    \boldsymbol{a}^{c,t} = \sum_{j \in \Gamma_{t}} \boldsymbol{a}^{s,j}
\]
and asserting the constraint
\[
    \left\langle \boldsymbol{a}^{c,t} , \boldsymbol{x} \right\rangle = 1
\]
...
trellis whose $t$th frame has $\binom{N_{t}}{L}\binom{N_{t+1}}{L}L!$ where $N_{t}$
and $N_{t+1}$ are the number of nodes in two consecutive frames of the original
