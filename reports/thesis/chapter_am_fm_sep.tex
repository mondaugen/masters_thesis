...
perhaps partitioning the data sets using local maxima and minima as illustrated
...
\begin{figure}[!t]
    %\centering
    \includegraphics[width=\textwidth]{plots/{ad_hoc_cluster_part_ex}.eps}
    \caption{The data points are convolved with some smoothing kernels giving a
        function with a small number of extrema. The maxima are indicated by
        circles and the minimum by a square. Two normal distributions are fit
        using the maxima as means and the minimum as a point some number of
        standard deviations away from that mean. This number of standard
        deviations is computed using the difference in height between the
        probability at the mean and that at the minimum.
        % NEED TO DO PLOT RIGHT
