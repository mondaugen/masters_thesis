...
The first prefers
smoothness in frequency between two frames. For frame $t$ with initial
classification $\tilde{p}$ we have frequency measurements
$\omega_{k,\tilde{p}}^{t}$
and frequency slope measurements $\psi_{k,\tilde{p}}^{t}$. The set of parameters
at time $t$ from initially classified source $\tilde{p}$ we will denote
$\boldsymbol{\theta}_{\tilde{p}}^{t}$. Between frame $t$ and frame $t-1$ we use
Algorithm~\ref{alg:mq_peak_match} on the pairs $\left\{ \boldsymbol{\theta}_{\tilde{m}}^{t},
\boldsymbol{\theta}_{\tilde{n}}^{t+1} \right\}$ with $(m,n) \in \{0,1\} \times
\{0,1\}$. For
each pair, $L$ is set to $\min(\# \boldsymbol{\theta}_{\tilde{m}}^{t} ,
\# \boldsymbol{\theta}_{\tilde{n}}^{t+1})$\footnote{Here, the threshold parameter $\Delta
...
frame, i.e.,
\[
    \mathcal{D}_{f} \left( \theta_{i,\tilde{m}}^{t},
    \theta_{j,\tilde{n}}^{t+1} \right) = | \omega_{i,\tilde{m}}^{t} +
    \psi_{i,\tilde{m}}^{t} H - \omega_{j,\tilde{n}}^{t+1} |
\]
where $H$ is the
...
It is given as
\[
    \mathcal{D}_{a} \left( \theta_{i,\tilde{m}}^{t},
    \theta_{j,\tilde{n}}^{t+1} \right) = | \log(A_{i,\tilde{m}}^{t}) +
    \psi_{i,\tilde{m}}^{t} H - \log(A_{j,\tilde{n}}^{t+1}) |
\].
We have found the absolute error to
give better results than the squared error.

The costs of these connections
are summed over the index pairs $\Gamma_{t}$ to give the entries of the cost
vector $\boldsymbol{c}$ in the LP
\[
    \boldsymbol{c}_{4t + 2m + n} = \sum_{i^{\ast},j^{\ast} \in \Gamma_{t}}
    \mathcal{D} \left( \theta_{i^{\ast},\tilde{m}}^{t}, \theta_{j^{\ast},\tilde{n}}^{t+1}
    \right)
\].
...
\begin{figure}
    %\centering
    \includegraphics[width=\textwidth]{plots/{hsrp_test_7_0.001_0.00001_smooth_freq_amp_sol}.eps}
    \caption{The
    \label{plot:hsrp_test_7_0.001_0.00001_smooth_freq_amp_sol}}
\end{figure}
The estimated sources after smoothing in frequency or amplitude are shown in
Figures~\ref{}
