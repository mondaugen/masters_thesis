...
There are two general audio source separation strategies that can be considered
as forming a continuum of techniques that are some hybrid of the two: those that
use physical or structural models of the sources are at one end of this
continuum and at the other end are purely statistical methods. Here we give a
...
thesis and its use is justified in Chapter~\label{chap:sigmod}.
...
purposes of upmixing and noise removal. We will discuss the technique in a bit
more detail to demonstrate how it differs from the additive technique explored
in this thesis.
...
\]
to the divergence to penalize matricies with large entries. A variety of
regularization terms are possible \cite[ch.~3]{cichocki2009nonnegative}.

%%% Add why doesn't model non-stationary audio well
